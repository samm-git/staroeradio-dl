...
my $content = get("http://server.audiopedia.su:8888/".
	(length($site)?'getapmp3parms':'getmp3parms').
	".php?mp3id=".$mp3id.(length($site) ? "&site=$site" : ""));
...
# finally run ffmpeg, without shell
system("ffmpeg", "-rtmp_playpath", "mp3:".$ref->{dir}."/$reqname",
	"-i", "rtmp://server.audiopedia.su/vod/", "-y", "-acodec", "copy",
	"-metadata", "title='$ref->{fullname}'", "-metadata", "artist='$domain'", 
	$savefname);
